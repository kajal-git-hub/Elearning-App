...
class SubjectContentFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val subFolderList = object : TypeToken<List<FindCourseFolderProgressQuery.SubfolderDuration>>() {}.type
        val subFoldersList: List<FindCourseFolderProgressQuery.SubfolderDuration> = gson.fromJson(subFolders, subFolderList)
        ...
        binding.clTopicType.setOnClickListener {
            ...
            bottomSheet.setOnTopicTypeSelectedListener(object : OnTopicTypeSelectedListener {
                override fun onTopicTypeSelected(selectedTopic: TopicTypeModel) { selectedTopic
                ...
                is com.student.competishun.di.Result.Success -> {
                    ...
                    when {
                        !subfolderDurationFolders.isNullOrEmpty() -> {
                            ...
                            val folderNames = subfolderDurationFolders.mapNotNull { it.folder?.name }
                            val folderCounts = subfolderDurationFolders.mapNotNull { it.folder?.folder_count ?: 0 }
                            val scheduledTimes = subfolderDurationFolders?.mapNotNull { it.folder?.scheduled_time ?: 0 }
                        ...
                        !folderProgressContent.isNullOrEmpty() -> {
                            ...
                            binding.rvSubjectContent.adapter = TopicContentAdapter(subjectContentList, folderId) { topicContent, folderContentId ->
                                when (topicContent.fileType) {
                                    ...
                                    "PDF" -> helperFunctions.showDownloadDialog(requireContext(), topicContent.url, topicContent.topicName)
